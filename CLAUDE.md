...
**Angle brackets when writing issue bodies:** GitHub's HTML pipeline truncates an issue/PR body at the first literal opening angle bracket — and code fences do **not** protect it (a `# <title>` inside a ```` ```md ```` block still cuts the body off there). Inline code spans only survive because the `<` gets HTML-escaped, which is fragile. So when you author or edit a body that needs to show an angle-bracket token (e.g. the template title marker), do **not** write the raw character anywhere — use the square-bracket form `[title]` and add a one-line note that the real on-disk/code token is the angle-bracket form. Always re-fetch the body after writing and confirm it isn't truncated.
