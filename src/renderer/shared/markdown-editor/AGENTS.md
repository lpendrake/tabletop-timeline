...
- `relationshipDirectives` — `{ library, defaultReason, onOpenNote?, onEditField?, bubbles? }`. Omit to still render directive blocks (built-in tracks only, `defaultReason: 'Unspecified'`) with no field-editing/note-opening callbacks. Clicking a blank (or Enter on a selected block) opens the built-in floating fill-in bubble by default; `onEditField` is an escape hatch for a host that wants to handle field-editing itself instead. `bubbles` supplies the note/option picker data (`noteOptions`, `defaultHolderId`, `currentNoteId`, `onHolderChosenWithoutDefault`, `createOption`, `heldOptions`) the bubble needs — see `extensions/AGENTS.md`.
