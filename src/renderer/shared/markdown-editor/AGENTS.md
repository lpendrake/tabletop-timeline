...
- `extensions/relationship-directives.ts` — renders `{{trackId.action ...}}` directives (parsed by `src/shared/relationships/`) as atomic, form-like blocks. Activated via `props.relationshipDirectives`; live mode only. See `extensions/AGENTS.md` for why it uses `Decoration.replace`, unlike wiki links. Reads no directive identity from the DOM — every click handler is a closure set in the widget's own `toDOM(view)`, and resolves its directive's CURRENT position via `view.posAtDOM(root)`, never a `data-*` attribute or a captured number.
- `extensions/relationship-bubble-logic.ts` / `relationship-bubble-state.ts` / `relationship-bubble.tsx` / `relationship-bubble-view-plugin.ts` — the floating fill-in bubble for a directive's blanks. See `extensions/AGENTS.md`'s "Relationship directives" section for the split.
...
- `relationshipDirectives` — `{ library, defaultReason, onOpenNote?, place?, bubbles? }`. Omit to still render directive blocks (built-in tracks only, `defaultReason: 'Unspecified'`) with no note-opening callback. Clicking a blank (or Enter on a selected block) always opens the built-in floating fill-in bubble — there is no escape hatch to handle field-editing another way. `place` (`'note' | 'event'`, default `'event'`) says whether this document is undated — passed to `interpretDirective` as `undated: place === 'note'`, which rejects Change/Shift/Remove on a note (see `src/shared/relationships/AGENTS.md`). `bubbles` supplies the note/option picker data (`RelationshipBubbleOptions`: `noteOptions`, `defaultHolderId`, `currentNoteId`, `onHolderChosenWithoutDefault`, `createOption`, `heldOptions`) the bubble needs — see `extensions/AGENTS.md`.
