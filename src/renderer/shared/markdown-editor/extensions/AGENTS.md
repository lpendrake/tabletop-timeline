...
- Field-editing (the floating fill-in bubble) is a later task. This extension only calls `onEditField({ from, ordinal }, role)` / `onOpenNote(id)` hook points; it renders no popover itself.
