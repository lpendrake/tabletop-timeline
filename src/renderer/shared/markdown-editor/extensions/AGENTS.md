...
- Pure, unit-tested helpers: `firstEditRole(parts)` (which blank a click on the wording targets), `crossEnd(fraction, previous)` (the delete-cross hysteresis, switching only past 60%/40% so it doesn't flicker near the middle), and `directiveBorderClass(status)` (below).
...
- **Ctrl/Cmd+click on a holder/observer value reliably opens its note.** Because the block is an atomic `Decoration.replace` range with `ignoreEvent() === false`, CodeMirror handles the mousedown itself first — snapping the selection to cover the whole block — before the browser's click reaches the value span's own listener; racing that against real click delivery is what made Ctrl/Cmd+click unreliable. `relationshipDirectives()` applies `makePointerGuard('.cm-directive-value')` (the same helper and default ctrl/meta-gated behaviour `wiki-links.ts` uses for `.cm-note-link`) so the modified pointerdown is swallowed in the capture phase — CodeMirror never gets to move the caret — and the existing click handler in `buildValueSpan` (which already checks `ctrlKey`/`metaKey` before falling through to opening the bubble) fires reliably. Plain (unmodified) pointerdown is left alone, so opening the fill-in bubble on a plain click is unaffected.
