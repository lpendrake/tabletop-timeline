...
- Clicking a blank (or Enter on a selected block) opens the built-in fill-in bubble via `openDirectiveBubble(view, directive.from, role)` (from `relationship-bubble-state.ts`) directly — no host callback needed. `onEditField` is only consulted as an escape hatch when a host explicitly supplies it.
...
- `wiki-links.ts` exports `directiveRanges(text)` (backed by the shared parser, not by this file) so its own `buildDecorations` can skip `[[id]]` occurrences that live inside a directive's role tokens — importing this extension's own code back into `wiki-links.ts` would be circular, since this file already imports `entityLabelMapField` from there.
